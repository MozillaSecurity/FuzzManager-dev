<template>
  <div id="main" class="panel panel-default">
    <div class="panel-heading">
      <i class="bi bi-file-bar-graph"></i> Report Summary -
      <span>{{ chart.data.created }}</span>
    </div>
    <template v-if="loading">
      ...
      <template v-if="loading_incomplete">
        <div class="loader-msg">
          <span
            >The requested summary is being generated by the server, please
            wait...</span
          >
        </div>
      </template>
    ...
    <template v-else>
      <div class="panel-body">
        <button class="btn btn-default" @click="refresh()">
          Refresh this Report Summary
        </button>
        <button class="btn btn-default" @click="textual()">
          View Textual Report
        </button>
      </div>
      <div class="panel-body">
...
export default defineComponent({
  ...
  setup() {
    ...
    const loading_incomplete = ref(false);
    ...
    const fetch = _.throttle(function () {
      loading.value = true;
      window
        .fetch(instance.proxy.URLS.rs_api, {
          method: "GET",
        ...
        .then((response) => {
          if (response.ok) {
            if (response.status === 202) {
              ...
              loading_incomplete.value = true;
              return;
            }
            loading_incomplete.value = false;
            loading.value = false;
        ...
        .then((json) => {
          if (json) {
            if ("error" in json) {
              swal("Configuration Error", json["error"], "error");
              return;
            }

            if ("warning" in json) {
              swal("Configuration Warning", json["warning"], "warning");
            }

            chart.slider.initialized = false;
            chart.data = json;

            const formattedDate = formatClientTimestamp(
              instance.proxy.collectionCreatedDate,
            );

            chart.data.created = formattedDate;
    ...
    const textual = function () {
      window.open(instance.proxy.URLS["html_url"], "_blank").focus();
    };

    const draw = function () {
      const self = instance.proxy;
      let data = self.chart.data;
      if (!data) return;
      ...
      let step = d3.scaleLinear().domain([1, 8]).range([1, 100]);
      ...
      function createTopLabelData(d) {
        ...
        if (id.length > max_len) {
          ...
          for (let sid of splitid) {
            if (
              new_splitid.length == 0 ||
              new_splitid[new_splitid.length - 1].length + sid.length > max_len
            ) {
      ...
      cell
        ...
        .attr("y", function (d, i) {
          return (
            15 /* Distance to top edge */ +
            i * 12 /* Should be equal to font size */
          );
        })
        .text(function (d) {
          return d;
    ...
    watch(
      () => chart.maxlevel,
      () => {
        if (!chart.slider.initialized) {
          chart.level = chart.maxlevel;
          chart.slider.initialized = true;
        }
      },
    );
  ...
  data() {
    let URLS = {
      rs_api: `/covmanager/collections/${this.collectionId}/summary/api/`,
      html_url: `/covmanager/collections/${this.collectionId}/summary/htmlexport/`,
    };
    return {
      csrftoken: "",
      URLS,
  ...
  async created() {
    ...
    const formattedDate = formatClientTimestamp(this.collectionCreatedDate);
    this.chart.data.created = formattedDate;
